...
async function getFileData(file, dirPath, serverAddress, serverPort) {
	...
	const url = `http://${serverAddress}:${serverPort}/img/${relativePath}`;
	let width = null;
	let height = null;
...
exploreDirectory('/ścieżka/do/folderu', '/ścieżka/do/folderu', 'adres.serwera', 'port').then(
	(json) => console.log(JSON.stringify(json, null, 2))
);
